...
class ItemLabelNode : SKNode
{
    ...
    var height:CGFloat {
        return label.frame.size.height +  valueLabel.frame.size.height + 5
    }
    var width:CGFloat {
    ...
    private let LINE_WIDTH:CGFloat = 0
...
//    }
    ...
    init( title:String, value:String, preferredMaxLayoutWidth:CGFloat) {
        ...
        valueLabel.position = CGPoint(x: 0, y: label.frame.size.height)
        valueLabel.fontSize = 12
        valueLabel.preferredMaxLayoutWidth = preferredMaxLayoutWidth
        
        let rectangle = SKShapeNode(rect: CGRect(x: 0, y: -label.frame.size.height/2, width: width, height: height), cornerRadius: 5)
